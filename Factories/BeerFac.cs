...
namespace DuserORM_MySQL
{
    ...
    {
        string sql = "SELECT B.ID, B.Name, C.Name AS CategoryName FROM Beer AS B INNER JOIN Category as C ON B.Category_id = C.ID ORDER BY C.Name";
        ...
        {
            using(MySqlCommand cmd = new MySqlCommand(sql,Conn.CreateConnection()))
            {
