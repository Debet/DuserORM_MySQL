...
namespace Duser
{
    public class AutoFac<T> where T : new()
    {
        ...
        public T Get(int ID)
        {
            using (var cmd = new MySqlCommand("SELECT * FROM " + table + " WHERE ID=?ID", Conn.CreateConnection()))
            {
        ...
        {
            using (var cmd = new MySqlCommand("SELECT * FROM " + table, Conn.CreateConnection()))
            {
        ...
        {
            using (var cmd = new MySqlCommand("SELECT * FROM " + table + " ORDER BY " + column + " " + direction, Conn.CreateConnection()))
            {
                List<T> list = mapper.MapList(cmd.ExecuteReader());
        ...
        {
            using (var cmd = new MySqlCommand("SELECT * FROM " + table + " ORDER BY " + column + " " + direction + " LIMIT " + amount, Conn.CreateConnection()))
            {
                List<T> list = mapper.MapList(cmd.ExecuteReader());
                cmd.Connection.Close();
                return list;
        ...
        //{
        //    using (var cmd = new MySqlCommand("SELECT * FROM " + table + " WHERE " + field + "=?KID", Conn.CreateConnection()))
        //    {
        //        cmd.Parameters.Add("?KID", value);
        ...
        {
            using (var cmd = new MySqlCommand("SELECT * FROM " + table + " WHERE " + column + "=?KID", Conn.CreateConnection()))
            {
        ...
        {
            using (var cmd = new MySqlCommand("SELECT * FROM " + table + " WHERE " + column + "=@KID ORDER BY " + ordercolumn + " " + direction, Conn.CreateConnection()))
            {
                cmd.Parameters.AddWithValue("@KID", value);
        ...
        public int Insert(T pro)
        {
            ...
            using (var cmd = new MySqlCommand("INSERT INTO " + table + " (" + columns + ") VALUES(" + parms + ");SELECT LAST_INSERT_ID() as curID;", Conn.CreateConnection()))
            {
        ...
        public void Update(T pro)
        {
            ...
            using (var cmd = new MySqlCommand("UPDATE " + table + " SET " + fAndP + " WHERE ID=@Id", Conn.CreateConnection()))
            {
        ...
        {
            using (var cmd = new MySqlCommand("UPDATE " + table + " SET " + column + "=?value WHERE ID=?ID", Conn.CreateConnection()))
            {
                cmd.Parameters.AddWithValue("?ID", ID);
    ...
    //        foreach (var prop in mappings)
    //        {
    //            cmd.Parameters.Add(prop.Key, pro.GetType().GetProperty(prop.Key).GetValue(pro, null));
    //        }
    ...
    public void Delete(int ID)
        {
            using (var cmd = new MySqlCommand("DELETE FROM " + table + " WHERE ID=@?ID", Conn.CreateConnection()))
            {
        ...
        {
            using (var cmd = new MySqlCommand("DELETE FROM " + table + " WHERE " + field + "=?value", Conn.CreateConnection()))
            {
        ...
        public int Count()
        {
            using (var cmd = new MySqlCommand("SELECT COUNT(ID) as antal FROM FROM " + table, Conn.CreateConnection()))
            {
                int counter = 0;
                var r = cmd.ExecuteReader();
        ...
        {
            using (var cmd = new MySqlCommand("SELECT COUNT(*) FROM FROM " + table + " WHERE " + column + "=@KID", Conn.CreateConnection()))
            {
                cmd.Parameters.AddWithValue("@KID", value);
                int counter = 0;
                var r = cmd.ExecuteReader();
        ...
        {
            using (var cmd = new MySqlCommand(SQL, Conn.CreateConnection()))
            {
